...
void rouletteShow() {
	...
	while (n != 1) {
		...
		for (i = location; i < location + shootNum; i++);//找到每轮退出的人的位置（i-1 才是，此处求得的i 值为下一轮开始的位置）
		i = i % n;//由于参与者排成的是环，所以需要对求得i 值进行取余处理
		if (i == 1 || i == 0) {//当i=1 或者i=0 时，实际上指的是位于数组开头和结尾的参与者，需要重新调整i 的值
			i = n + i;
		}
