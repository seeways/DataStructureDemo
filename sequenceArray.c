...
Status sequenceArrayValue(ElemType* e, sequenceArray A, ...) //在VC++中，...之前的形参不能是引用类型
{
	...
	int off;
	va_start(ap, A);
	if ((result = sequenceArrayLocate(A, ap, &off)) == OVERFLOW) //调用Locate()
		return result;
	*e = *(A.base + off);
	return OK;
}
...
void execSequenceArray() {
	...
	ElemType e, * p1;
	InitSequenceArray(&A, dim, bound1, bound2, bound3); //构造3＊4＊2的3维数组A
	p = A.bounds;
	printf("A.bounds=");
	...
	for (i = 0; i < bound1; i++)
	{
		for (j = 0; j < bound2; j++)
		{
			for (k = 0; k < bound3; k++)
			{
				SequenceArrayAssign(&A, i * 100 + j * 10 + k, i, j, k); // 将i*100+j*10+k赋值给A[i][j][k]
				sequenceArrayValue(&e, A, i, j, k); //将A[i][j][k]的值赋给e
